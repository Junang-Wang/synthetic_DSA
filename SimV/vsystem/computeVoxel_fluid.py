            ...
            if exz < 0:
                ...
                if int(x) >= 0 & int(x) < tVol[0]:
                    img_stack = diamVoxels(
                        x, y, z, 0, r, value, img_stack, tVol, res=resolution
                    )
            else:
                z += sz
                exz -= bx
                ...
                if int(z) >= 0 & int(z) < tVol[2]:
                    img_stack = diamVoxels(
                        x, y, z, 2, r, value, img_stack, tVol, res=resolution
                    )
        else:
            if ezy < 0:
                z += sz
                ...
                if int(y) >= 0 & int(y) < tVol[1]:
                    img_stack = diamVoxels(
                        x, y, z, 1, r, value, img_stack, tVol, res=resolution
                    )
...
def discretisation(p1, dmin, dmax, tVol):
    ...
    return (
        np.array([x, y, z, p1[3], p1[4]]).reshape(-1, 1)
        if len(p1) > 4
        else np.array([x, y, z, p1[3]]).reshape(-1, 1)
    )
